...
bool vk_loader::is_device_suitable(VkPhysicalDevice device) {
  ...
  if (check_device_extension_support(device)) {
    ...
    swapchain_suitable = !swapchain_support.formats.empty() &&
                         !swapchain_support.present_modes.empty();
  }

  if (!swapchain_suitable)
    return false;
...
vk_loader::query_swapchain_support(VkPhysicalDevice device) {
...
void vk_loader::create_swapchain(GLFWwindow *window) {
  swapchain_support_details swapchain_support =
      query_swapchain_support(m_selected_physical_device);
...
void vk_loader::create_swapchain_image_views() {
...
void vk_loader::create_def_graphics_pipeline() {
  ...
  if (vkCreateGraphicsPipelines(m_logical_device, VK_NULL_HANDLE, 1,
    ...
    throw std::runtime_error("failed to create graphics pipeine");
  }
}

void vk_loader::create_render_pass() {
  ...
  if (vkCreateRenderPass(m_logical_device, &render_pass_info, nullptr,
                         &m_render_pass) != VK_SUCCESS) {
    throw std::runtime_error("Failed to create render pass");
  }
}

void vk_loader::create_framebuffers() {
  ...
  for (size_t i = 0; i < m_swapchain_image_views.size(); ++i) {
    ...
    if (vkCreateFramebuffer(m_logical_device, &framebuffer_info, nullptr,
...
void vk_loader::create_vertex_buffer(const std::vector<vertex> *vertices) {
  ...
  VkDeviceMemory staging_buffer_memory; // Temporary buffer to copy the memory
                                        // to the gpu. The final buffer will be
                                        // only accesible from the GPU side
...
VkSwapchainKHR vk_loader::get_swapchain() { return m_swapchain; }
VkExtent2D vk_loader::get_swapchain_extent() { return m_swapchain_extent; }
VkPipeline vk_loader::get_graphics_pipeline() { return m_graphics_pipeline; }
VkQueue vk_loader::get_graphics_queue() { return m_graphics_queue; }
VkQueue vk_loader::get_present_queue() { return m_present_queue; }
...
void vk_loader::create_descriptor_pool() {
  ...
  // Number of descriptor sets needed usually matches the number of frames in
...
void vk_loader::create_descriptor_sets(VkImageView image_view,
                                       VkSampler tex_sampler) {
  // Determine how many descriptor sets we need (usually equal to the number
...
VkImageView vk_loader::create_image_view(VkImage image, VkFormat format) {
  ...
  if (vkCreateImageView(m_logical_device, &view_info, nullptr, &image_view) !=
      VK_SUCCESS) {
